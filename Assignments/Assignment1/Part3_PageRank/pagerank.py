...
ranks = documents.groupByKey().mapValues(lambda x : 1)

ITERATIONS = 10
for i in range(ITERATIONS):
	#Build an RDD of (targetURL, float) pairs // with the contributions sent by each page 
	contribs = documents.join(ranks).flatMap(lambda url_rank : documents.map(lambda url_rank : (url_rank[1][0], url_rank[1][1]/len(url_rank[1]))))
